...
class Pirate:
    ...
    def step_change(self):
        """Mover y rebotar en la caja 0..200."""
        self.xpos += self.vx
        self.ypos += self.vy
        ...
        if self.xpos < 0:
            self.xpos = 0
            self.vx *= -1
        if self.xpos + self.width > 200:
            self.xpos = 200 - self.width
            self.vx *= -1
        ...
        if self.ypos < 0:
            self.ypos = 0
            self.vy *= -1
        if self.ypos + self.height > 200:
            self.ypos = 200 - self.height
            self.vy *= -1
