...
class Terrain:
    def __init__(
        ...
        spawns: Optional[Iterable[Tuple[int, int]]] = None,
        exits: Optional[Iterable[Tuple[int, int]]] = None,
    ) -> None:
        self.width = width
        self.height = height
        ...
        self._ride_bboxes: List[Tuple[int, int, int, int]] = []
        self._baseline_grid: Optional[List[List[int]]] = None
        self._baseline_spawns: Optional[List[Tuple[int, int]]] = None
        self._baseline_exits: Optional[List[Tuple[int, int]]] = None
        self._baseline_bboxes: Optional[List[Tuple[int, int, int, int]]] = None
    ...
    @classmethod
    def from_size(cls, width: int, height: int) -> "Terrain":
        grid = []
    ...
    def from_definition(
        ...
        height: int,
        *,
        obstacles: Optional[Iterable[Iterable[int]]] = None,
        entrances: Optional[Iterable[Tuple[int, int]]] = None,
        exits: Optional[Iterable[Tuple[int, int]]] = None,
        border: bool = True,
    ) -> "Terrain":
        grid = []
        ...
        for bbox in obstacles or []:
            terrain.add_bbox_barrier(tuple(int(value) for value in bbox))

        return terrain

    def is_free_point(self, point: Tuple[float, float]) -> bool:
        x = int(point[0])
        y = int(point[1])
    ...
    def is_free_line(self, start: Tuple[float, float], end: Tuple[float, float]) -> bool:
        steps = 6
    ...
    def near_bbox(self, point: Tuple[float, float], bbox, tol: float = 0.5) -> bool:
        x, y, w, h = bbox
        px, py = point
        return (x - tol) <= px <= (x + w + tol) and (y - tol) <= py <= (y + h + tol)

    def random_free_point(self) -> Tuple[int, int]:
        while True:
    ...
    def add_bbox_barrier(self, bbox) -> None:
        for xx, yy in self._cells_for_bbox(bbox):
            if 0 <= yy < self.height and 0 <= xx < self.width:
                self.grid[yy][xx] = 1

    def add_ride(self, ride, *, allow_tangent=True) -> None:
        bbox = ride.bbox
    ...
    def reset(self) -> None:
        ...
        baseline = cast(List[Tuple[int, int, int, int]], self._baseline_bboxes or [])
        self._ride_bboxes = [
            (
                int(bbox[0]),
                int(bbox[1]),
            ...
            for bbox in baseline
        ]

    def _bbox_inside(self, bbox) -> bool:
        x, y, w, h = bbox
        return 0 <= x and 0 <= y and (x + w) <= self.width and (y + h) <= self.height

    def _bbox_intersects(self, a, b, *, allow_tangent=True) -> bool:
        ax, ay, aw, ah = a
        bx, by, bw, bh = b
        if allow_tangent:
