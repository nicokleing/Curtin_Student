"""Handle exporting simulation results to files."""
    ...
    def _create_readme(self, files_created: List[str]) -> Optional[str]:
        try:
            ...
            content = (
                ...
                f"Files saved: {', '.join(Path(path).name for path in files_created)}\n"
                "Open events.csv or summary.json with any spreadsheet tool.\n"
                "Reuse summary.json values to repeat the run.\n"
            )
